...
redirect_primary_dict = dict()
redirect_detour_dict = dict()
...
def add_redirect_primary_entry(redirect_primary_dict,datapath_id,req,flow_entry):
    if (datapath_id,req) not in redirect_primary_dict.keys():
       redirect_primary_dict = dict(redirect_primary_dict.items() + [((datapath_id,req), [flow_entry])])
    else:
       redirect_primary_dict[(datapath_id,req)].append(flow_entry)
    return redirect_primary_dict

def add_redirect_detour_entry(redirect_detour_dict,datapath_id,req,fault,flow_entry):
    if (datapath_id,req,fault) not in redirect_detour_dict.keys():
       redirect_detour_dict = dict(redirect_detour_dict.items() + [((datapath_id,req,fault), [flow_entry])])
    else:
       redirect_detour_dict[(datapath_id,req,fault)].append(flow_entry)
    return redirect_detour_dict
...
def generate_flow_entries_dict(GUI=False):
    ...
    global redirect_detour_dict
    global redirect_primary_dict
    global fault_ID
    global group_ID
    ...
    for i in range(1,len(faults.keys())+1):
        a = faults.keys()[i-1]
        inv_a = (a[1],a[0])
        fault_ID = dict(fault_ID.items() + [(a, i)])
        fault_ID = dict(fault_ID.items() + [(inv_a, i)])
    ...
    for i in range(len(requests)):
        ...
        for x in range(len(primary_path)):
            ...
            # match(SRC_MAC, DEST_MAC, in_port) -> action(output(next_primary_hop))
            ...
            if primary_path[x] not in detect_nodes and primary_path[x] not in redirect_nodes :
                ...
                if x == 0: # first node in the primary path
                    ...
                    flow_entry = dict()
                    flow_entry['match']=ofparser.OFPMatch(in_port=mn_topo_ports['s'+str(primary_path[x])]['h'+str(primary_path[x])],
                        eth_src=int_to_mac_str(request[0]),eth_dst=int_to_mac_str(request[1]),eth_type=0x8847,metadata=16)
                ...
                if x == 0: # first node in the primary path
                    ...
                    flow_entry['inst']=[ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, flow_entry['actions'])]
                    flow_entry['table_id']=0
                    redirect_primary_dict = add_redirect_primary_entry(redirect_primary_dict,primary_path[x],request,flow_entry)
                    flow_stats_dict = update_flow_stats(flow_stats_dict,primary_path[x],'primary')
                ...
                #[DETECT ONLY]
                '''
                se un edge node e' detect => non puo' non essere di redirect! -> quindi i detect only non possono essere edge nodes
                '''
                #bucket creation (go to the next primary node)
                max_len = 2000
                actions = [ofparser.OFPActionOutput(mn_topo_ports['s'+str(primary_path[x])]['s'+str(primary_path[x+1])],0)]
                ...
                succedere che io sia solo redirect. l'unico modo sarebbe non gestire un fault di un certo link:
                ...
                if x == 0: # first node in the primary path
                    ...
                    # partendo da f_t_parser_ctrl_drop, l'idea e' usare la Flow Table 1 come Group Table fitizia che usa Flags al posto di FastFailover
                    flow_entry = dict()
                    flow_entry['match']=ofparser.OFPMatch(in_port=mn_topo_ports['s'+str(primary_path[x])]['h'+str(primary_path[x])],
                        eth_src=int_to_mac_str(request[0]),eth_dst=int_to_mac_str(request[1]),eth_type=0x8847,metadata=16)
                    flow_entry['actions']=[ofparser.OFPActionSetField(mpls_label=16),ofparser.OFPActionOutput(mn_topo_ports['s'+str(primary_path[x])]['s'+str(primary_path[x+1])],0)]
                    flow_entry['inst']=[ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, flow_entry['actions'])]
                    flow_entry['table_id']=1
                    flow_entries_dict = add_flow_entry(flow_entries_dict,primary_path[x],flow_entry)
        ...
        for y in range(len(requests[request]['faults'].items())):
            ...
            if l_d_n_index_in_p_p == len(primary_path)-1: # l.d.n. is an edge switch
                ...
                flow_entry['actions']=[ofparser.OFPActionOutput(mn_topo_ports['s'+str(detour[len(detour)-1])]['s'+str(primary_path[l_d_n_index_in_p_p + 1])],0),
                        ofparser.OFPActionSetField(mpls_label=16)]
            flow_entry['inst']=[ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, flow_entry['actions'])]
            flow_entry['table_id']=0
            ...
            if redirect_node == detect_node:
                # match(SRC_MAC, DST_MAC, in_port, global_state=LINK_DOWN) -> action(OUTPUT(DETOUR_PATH), TAG=ID_BROKEN_LINK)
                ...
                if node_index_in_p_p == 0: # Detect&Redirect node is an edge switch
                    ...
                    #bucket creation (drop packet when a fault is detected)
                    max_len = 2000
                    ...
                    weight = 0
                    watch_port = mn_topo_ports['s'+str(primary_path[node_index_in_p_p])]['h'+str(primary_path[node_index_in_p_p])]
                    watch_group = ofproto.OFPG_ANY
                    bucket = ofparser.OFPBucket(weight, watch_port, watch_group,actions)
                ...
                if node_index_in_p_p == 0: # Redirect only node is an edge switch
                    ...
                    flow_entry['table_id']=0
                    redirect_detour_dict = add_redirect_detour_entry(redirect_detour_dict,redirect_node,request,fault[0],flow_entry)
                ...
                #match(SRC_MAC, DST_MAC, in_port, global_state=LINK_DOWN) -> action(TAG(ID_BROKEN_LINK), output(FWD_BACK_PATH))
